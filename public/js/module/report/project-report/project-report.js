...
$('#btn-preview').on('click', function(){
    ...
    if(isValid){
        ...
        apiCall('report/project-report/data/preview', 'POST', 'project-report', 
...
$('.btn-print').on('click', function(){
    ...
    if(isValid){
        // Call API to show preview
        apiCall(`report/project-report/data/${mode}`, 'POST', 'project-report', 
            {"X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content")},
        null, 
        null,
        true,
        (res) => {
            ...
            // window.open(`/report/project-report/download-file/${fileName}`, '_blank');
            window.location.href = `/report/project-report/print/${fileName}`
            setTimeout(() => {
                Toastify({
                    text: `Berhasil Mencetak Report Project`,
                    duration: 1000,
                    close:true,
                    gravity:"top",
                    callback: function() {
                        renderView(`${$('meta[name="baseurl"]').attr('content')}report/project-report`)
                    },
                    position: "right",
                    style: {
...
function validation(){
    const fAddComponent = $('#project-report')
